...
def build_voiceover_prompt(input_data: dict, has_images: bool, image_analysis: str = '') -> str:
    ...
    if tone == 'tiktok-nhay':
        ...
        _struct_check = [
            ...
            "- Có bịa deal/giảm giá/flash sale/kẻo hết hàng/kẻo hết size không có trong mô tả? → Xóa.",
            "- CTA có bịa scarcity ('kẻo hết', 'sắp hết'), so sánh giá ('rẻ hơn chỗ khác') không trong mô tả? → Xóa.",
            f"- Hook có đúng kiểu '{_hook_short}' không? → Nếu không, viết lại hook.",
            f"- Cấu trúc có đúng '{_pattern_short}' không? → Nếu lệch, điều chỉnh.",
    ...
    lines = [
        ...
        "Tối đa 1 reaction liên tiếp — không được viết 2-3 reaction ngắn liền nhau không có ý nào xen giữa.",
        "",
    ...
    lines += [
        ...
        *_struct_check,
        "",
        "## VIẾT CTA — LUẬT BẮT BUỘC",
        f"CTA ngành {industry_label}: {industry_data.get('cta', chr(34) + 'ai đang cần [giải pháp] thì thử nha' + chr(34) + ' / ' + chr(34) + 'link ở bio' + chr(34))}",
        "CTA PHẢI: tự nhiên, kết nối với nội dung vừa kể, mời tìm hiểu thêm.",
        "CTA KHÔNG ĐƯỢC: bịa 'kẻo hết hàng/size', 'flash sale', 'giảm giá X%', 'rẻ hơn chỗ khác' — nếu không có trong mô tả sản phẩm.",
        "",
        "---",
        "",
